...
const Fund = () => {
  ...
  return <section id="fund" className="py-16 px-6">
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="space-y-6 mb-12">
          <p className="font-sans text-xl leading-relaxed text-muted-foreground text-center max-w-3xl mx-auto">SRI Capital Fund 1 has been investing in early-stage technology companies across the US and India since 2018. We provide not just capital, but strategic guidance and network access to help founders scale their vision into reality. Fund 1 is currently not making new investments.</p>
        </div>

        <div className="border-t border-border/50 pt-10">
          <h3 className="font-serif text-4xl font-bold mb-12 text-center">Team</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-4xl mx-auto">
            {teamMembers.map(member => <div key={member.name} className="group text-center">
                <div className="overflow-hidden rounded-2xl border-2 border-border hover:border-primary/60 transition-all duration-500 shadow-[0_10px_40px_rgba(0,0,0,0.25)] hover:shadow-[0_0_30px_rgba(0,174,255,0.4),0_25px_60px_rgba(0,0,0,0.35)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.5)] dark:hover:shadow-[0_0_30px_rgba(0,174,255,0.5),0_25px_60px_rgba(0,0,0,0.6)] mb-6 w-[400px] h-[400px] mx-auto">
                  <img src={member.image} alt={`${member.name}, ${member.title} at SRI Capital`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                </div>
                ...
                <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 transition-colors whitespace-nowrap" aria-label={`Visit ${member.name}'s LinkedIn profile`}>
